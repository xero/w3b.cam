...
**`bun dev --index-only`.** Rebuilds only `index.html` and reuses the rest of `out/` from your last full bake, so startup drops from tens of seconds to about one. Every full bake writes an image manifest (`out/.img-manifest.json`) that lets this path pull the homepage's images off disk instead of re-extracting all of them. The tradeoff is freshness: the galleries, detail pages, and the homepage's own "newest" cards reflect the last full bake, not any DB changes since, so run a plain `bun dev` when you need those current. If the manifest is missing, it falls back to a full build once. Use it for fast restarts while iterating on homepage layout or featured picks.
