...
// a card or screenshot in the browser drives those endpoints (see src/server/dev-client/).
//
...
async function handleDev(req: Request, path: string): Promise<Response> {
	...
	// Dispatches on `type` to the SAME ingest core (src/ingest/ingest.ts) the CLI runs. Every
