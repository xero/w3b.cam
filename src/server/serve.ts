 ...
 * Exported so `bun dev` (src/server/dev.ts) can reuse the exact static logic and only layer
...
// (from src/server/dev.ts) does not, so the dev server owns the single listener on 1337.
if (import.meta.main) {
