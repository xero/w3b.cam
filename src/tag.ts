// Tag: attach a free-form metadata tag to a cam, stream, or traffic entry, stored in
// the unified `tags` table. `kind` selects the source and `ref` is that source's key:
// an IP (cam), a YouTube video id (stream), or an Osiris cam id (traffic). A single
...
//
// Usage:  bun run tag <cam|stream|traffic> <ref> <tag>

import { isIP } from "node:net";
import { addTag, closeDb, hasHost, hasStream, hasTraffic, openDb } from "./db.ts";
...
if ((kind !== "cam" && kind !== "stream" && kind !== "traffic") || !ref || !tag) {
  console.error("Usage: bun run tag <cam|stream|traffic> <ref> <tag>");
  process.exit(1);
}
...
try {
  ...
  const stored = kind === "cam" ? hasHost(db, ref) : kind === "stream" ? hasStream(db, ref) : hasTraffic(db, ref);
  console.log(`\n── Tag summary ──`);
  ...
  if (!stored) {
    const what = kind === "cam" ? "cameras" : kind === "stream" ? "stream" : "traffic cam";
