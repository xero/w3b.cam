// Unfeature: remove a cam/stream from the homepage's featured set (index.html). The
// inverse of `bun run feature`; `kind` selects the source and `ref` is that source's
// key (an IP or a YouTube video id). Re-run `bun run bake` afterwards to drop it from
// the homepage rotation. No API, no query credits.
//
// Usage:  bun run unfeature <cam|stream> <ref>
...
if ((kind !== "cam" && kind !== "stream") || !ref) {
  console.error("Usage: bun run unfeature <cam|stream> <ref>");
  process.exit(1);
}

// A cam is featured by IP; validate it so a typo can't silently match nothing. A
// stream (video id) ref is an opaque string, so accept any.
