 ...
 * tags / featured are seeded-then-curated.
 */
const YT_GEO_SCHEMA = `
...
/**
 * Unified free-form tags across all three sources, replacing the IP-only `ip_tags`.
 * `kind` selects the source and `ref` is that source's key: 'cam' -> webcams.ip_str
 * (host-level, shared across a host's ports, exactly as ip_tags was), 'stream' ->
 * youtube.video_id, 'traffic' -> traffic.id. Mirrors how `featured` references both
 * cams and streams from one table. Keyed on (kind, ref, tag) so a tag can't be
 * applied to the same entity twice. `tag` is stored normalized (see normalizeTag).
 * Existing ip_tags rows are copied in as kind='cam' on first open (backfillTags); a
 * fresh DB is seeded from IP_TAGS_SEED (seedTags).
 */
const TAGS_SCHEMA = `
CREATE TABLE IF NOT EXISTS tags (
	kind      TEXT NOT NULL,   -- 'cam' | 'stream' | 'traffic'
	ref       TEXT NOT NULL,   -- webcams.ip_str | youtube.video_id | traffic.id
	tag       TEXT NOT NULL,
	added_at  TEXT NOT NULL DEFAULT (datetime('now')),
...
/** Which source a tag's `ref` points at: an IP (cam), a YouTube video id (stream), or an Osiris cam id (traffic). */
export type TagKind = "cam" | "stream" | "traffic";

/**
 * Tag -> IPs used to seed the `tags` table (as kind='cam') on a fresh database.
 * Applied only when the table is empty (see seedTags), so a tag removed by hand
 * never comes back. Tags are normalized on insert, so casing/whitespace here is
 * cosmetic.
 */
 ...
 * by hand. Mirrors seedTags / seedFeatured.
 */
...
/**
 * One-shot migration of the retired `ip_tags` table into the unified `tags` table.
 * Copies every ip_tags row in as kind='cam' (its refs are ip_strs, exactly what
...
function backfillTags(db: Database): void {
	...
	if (!present) return;
	db.transaction(() => {
		db.run("INSERT OR IGNORE INTO tags (kind, ref, tag, added_at) SELECT 'cam', ip_str, tag, added_at FROM ip_tags");
		db.run("ALTER TABLE ip_tags RENAME TO ip_tags_migrated");
	})();
}

/**
 * Populate `tags` from IP_TAGS_SEED (all cams) on a fresh (empty) table. Idempotent:
 * once the table holds any row this is a no-op, so it never re-adds a tag removed by
 * hand. Runs after backfillTags, so on a migrated DB `tags` is already non-empty and
 * this no-ops. Mirrors seedHostBlacklist / seedFeatured.
 */
export function seedTags(db: Database): void {
	const { c } = db.query("SELECT COUNT(*) AS c FROM tags").get() as { c: number };
	if (c > 0) return;
	const stmt = db.query("INSERT OR IGNORE INTO tags (kind, ref, tag) VALUES ('cam', ?, ?)");
 ...
 * so the comma-joined display on a detail page is stable. Replaces loadIpTags;
 * loadTags(db, 'cam') reproduces its exact behavior.
 */
export function loadTags(db: Database, kind: TagKind): Map<string, string[]> {
	const rows = db
		.query("SELECT ref, tag FROM tags WHERE kind = ? ORDER BY ref, tag")
		.all(kind) as { ref: string; tag: string }[];
	const map = new Map<string, string[]>();
	for (const r of rows) {
		const list = map.get(r.ref);
		if (list) list.push(r.tag);
		else map.set(r.ref, [r.tag]);
	}
	return map;
}
 ...
 * before 'stream' before 'traffic'). The build resolves each ref against its
 * in-memory view models and skips any whose row is gone (see build.ts).
 */
export function loadTagIndex(db: Database): Map<string, { kind: TagKind; ref: string }[]> {
	const rows = db
		.query("SELECT kind, ref, tag FROM tags ORDER BY tag, kind, ref")
		.all() as { kind: TagKind; ref: string; tag: string }[];
	const map = new Map<string, { kind: TagKind; ref: string }[]>();
	for (const r of rows) {
		const entry = { kind: r.kind, ref: r.ref };
		const list = map.get(r.tag);
		if (list) list.push(entry);
		else map.set(r.tag, [entry]);
	}
	return map;
}
 ...
 * source and `ref` is that source's key (ip_str / video_id / traffic id). Replaces
 * addIpTag. Returns true if newly added, false if the entity already carried that tag
 * (or the tag normalizes to empty).
 */
export function addTag(db: Database, kind: TagKind, ref: string, tag: string): boolean {
	const t = normalizeTag(tag);
	if (t === "") return false;
	return db.query("INSERT OR IGNORE INTO tags (kind, ref, tag) VALUES (?, ?, ?)").run(kind, ref, t).changes > 0;
}

/**
 * Remove a single tag from an entity, normalized to match how it was stored (see
 * normalizeTag). Returns true if a row was deleted, false if the entity did not carry
 * that tag (or the tag normalizes to empty). The inverse of addTag.
 */
export function removeTag(db: Database, kind: TagKind, ref: string, tag: string): boolean {
	const t = normalizeTag(tag);
	if (t === "") return false;
	return db.query("DELETE FROM tags WHERE kind = ? AND ref = ? AND tag = ?").run(kind, ref, t).changes > 0;
}
...
export function entityTags(db: Database, kind: TagKind, ref: string): string[] {
	return (db.query("SELECT tag FROM tags WHERE kind = ? AND ref = ? ORDER BY tag").all(kind, ref) as { tag: string }[])
		.map((r) => r.tag);
}
...
export function distinctTags(db: Database): string[] {
	return (db.query("SELECT DISTINCT tag FROM tags ORDER BY tag").all() as { tag: string }[])
		.map((r) => r.tag);
}

/**
 * Every distinct tag with how many entities carry it, ordered by tag name. Since
 * `tags` is keyed on (kind, ref, tag), a tag can't repeat on one entity, so COUNT(*)
 * per tag is exactly its entity count across all three sources. Counts every tagged
 * ref whether or not its row still exists, so the cloud reflects the raw tagging
 * effort (a browse page may show fewer cards; see build.ts). Feeds renderTagsMain.
 */
...
/** True when a traffic cam with this exact id is stored. Companion to hasHost / hasStream. */
export function hasTraffic(db: Database, id: string): boolean {
	return db.query("SELECT 1 FROM traffic WHERE id = ? LIMIT 1").get(id) != null;
}
